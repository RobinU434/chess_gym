...
class ChessSpace(spaces.Space):
    def __init__(
        ...
        action_encoding: str = "action_wise",
    ) -> None:
    ...
    def _get_board_space(
        self, observation_mode, render_size: int = 512
    ) -> spaces.Space:
    ...
    def _get_possible_actions() -> Tuple[List, List]:
        """returns all permutations of chess board squares and all used symbols
        ...
        bar = Bar(
            ...
            max=(len(nums) ** 2 * len(letters) ** 2) - len(nums) * len(letters),
        )
        actions = []
    ...
    def sample(self) -> Tuple[torch.Tensor, torch.tensor]:
        ...
        action_sample = self.one_hot_encoding(
            random.choices(self.possible_actions, k=random.randint(1, 24))
        )
